...
export function redactValue(
	...
	if (Array.isArray(value))
		return value.map((entry) => redactValue(entry, depth + 1, hits, marker));
	if (isPlainObject(value)) {
		const out: Record<string, unknown> = {};
		for (const [k, v] of Object.entries(value)) {
			out[k] = redactValue(v, depth + 1, hits, marker);
		}
		return out;
	}
	return value;
...
export function redactCallToolResult(result: CallToolResult): CallToolResult {
	...
	if (result.structuredContent !== undefined) {
		const redacted = redactValue(result.structuredContent, 0, hits, marker);
